...
public class OverbookService {
	...
	public PostingRes postTransactionViaAPI(PostingReq request,
			Identity identity) {
		...
		dataJson.put(JsonVariable.SENDER_NAME, request.getPostingFrom()
				.getAccountName().toUpperCase());
		dataJson.put(JsonVariable.SENDER_ACCOUNT, request.getPostingFrom()
				.getAccountNumber());
		dataJson.put(JsonVariable.BENEFICIARY_NAME, request.getPostingTo()
				.getAccountName());
		dataJson.put(JsonVariable.BENEFICIARY_ACCOUNT, request.getPostingTo()
				.getAccountNumber());
		dataJson.put(JsonVariable.BENEFICIARY_BANK_CODE, request.getPostingTo()
				.getBankCode().toString());
		dataJson.put(JsonVariable.CURRENCY_CODE,
				BkpmConstants.CURRENCY_CODE_RUPIAH);
		dataJson.put(JsonVariable.AMOUNT, request.getAmount().toString());
		dataJson.put(JsonVariable.REFERENCE, "");
		dataJson.put(JsonVariable.CONFIRMATION_CODE,
				request.getConfirmationCode());

		Map<String, String> header = new HashMap<String, String>();
		header.put(BkpmConstants.HTTP_HEADER_CONTENTTYPE,
				BkpmConstants.HTTP_HEADER_CONTENT_JSON);
		header.put(BkpmConstants.HTTP_HEADER_ACCEPT,
				BkpmConstants.HTTP_HEADER_CONTENT_JSON);
		// only for prod
		...
		try {
			...
			if (res != null) {
				...
				if ("200".equals(statusCode)) {
					JSONObject dataObj = (JSONObject) responseJson
							.get(JsonVariable.DATA);
					if ("00".equals((String) dataObj
							.get(JsonVariable.RESPONSE_CODE))) {
						...
						result.setAuthID((String) dataObj
								.get(JsonVariable.AUTH_ID));
						result.setBankReference((String) dataObj
								.get(JsonVariable.BANK_REFFERENCE));
						result.setStan((String) dataObj.get(JsonVariable.STAN));
						result.setResponseCode((String) dataObj
								.get(JsonVariable.RESPONSE_CODE));
						result.setResponseDescription((String) dataObj
								.get(JsonVariable.RESPONCE_DESCRIPTION));
						result.setTransmissionDateTime((String) dataObj
								.get(JsonVariable.TRANSMISSION_DATE_TIME));

						// save data to DB
						FundTransfer tf = new FundTransfer();
						tf.setAccountNumber(request.getPostingFrom()
								.getAccountNumber());

						String adminFee = "";
						String transferType;
						if (BkpmConstants.BUKOPIN_BANK_CODE.equals(request
								.getPostingTo().getBankCode())) {
							// ntr d caching
							adminFee = config
									.getConfigValue("OVERBOOK_ADMIN_FEE");
							method = messageUtil.get("overbook",
									servletRequest.getLocale());
							transferType = BkpmConstants.OVERBOOK;

							tf.setAdminFee(new BigDecimal(adminFee));
							tf.setMethod(BkpmConstants.OVERBOOK);
							tf.setMenu(INHOUSE_TRANSFER);

							result.setAdminFee(new BigDecimal(adminFee));
							result.setMethod(messageUtil.get("overbook",
									servletRequest.getLocale()));
							result.setTransferType(BkpmConstants.OVERBOOK);
						} else {
							// ntr d caching
							adminFee = config
									.getConfigValue("FUND_TRANSFER_ADMIN_FEE");
							method = messageUtil.get("fund.transfer",
									servletRequest.getLocale());
							transferType = BkpmConstants.FUND_TRANSFER;

							tf.setAdminFee(new BigDecimal(adminFee));
							tf.setMethod(BkpmConstants.FUND_TRANSFER);
							tf.setMenu(DOMESTIC_TRANSFER);

							result.setAdminFee(new BigDecimal(adminFee));
							result.setMethod(messageUtil.get("fund.transfer",
									servletRequest.getLocale()));
						...
						// get user data
						String urlUser = config.getConfigValue("USER_BASE_URL")
								+ config.getConfigValue("URL_FINDUSER_BY_USERNAME");

						// use this if run user in local PC
						// String urlUser
						// ="http://localhost:8080/bukopinmobile-user/account/findUserByUsername";
						...
						identityObj.put(JsonVariable.REQ_TIME,
								identity.getReqTime());
						identityObj.put(JsonVariable.PLATFORM,
								identity.getPlatform());
						identityObj.put(JsonVariable.USER_AGENT,
								identity.getUserAgent());
						identityObj.put(JsonVariable.SECRET_CODE,
								identity.getSecretCode());
						identityObj
								.put(JsonVariable.TOKEN, identity.getToken());
						dataJson.put(JsonVariable.IDENTITY, identityObj);

						JSONObject parameter = new JSONObject();
						parameter.put(JsonVariable.USERNAME, request
								.getPostingFrom().getUsername());
						...
						res = ConnectionUtils.urlAccess(urlUser,
								dataJson.toString(), header).toString();
						...
						if (res != null) {
							...
							userObj = (JSONObject) responseJson
									.get(JsonVariable.RESULT);
							user.setId((Long) userObj.get(JsonVariable.ID));
							user.setCreatedBy((String) userObj
									.get(JsonVariable.CREATED_BY));
							...
							log.debug("json "
									+ (String) userObj
											.get(JsonVariable.CREATED_DATE));
							String dt = (String) userObj
									.get(JsonVariable.CREATED_DATE);
							log.debug("date format local " + dt);

							user.setCreatedDate(dateFormat.parse(dt));
							user.setEmail((String) userObj
									.get(JsonVariable.EMAIL));
							user.setFirstName((String) userObj
									.get(JsonVariable.FIRSTNAME));
							user.setLastName((String) userObj
									.get(JsonVariable.LASTNAME));
							user.setMiddleName((String) userObj
									.get(JsonVariable.MIDDLENAME));
							user.setMobilePhone((String) userObj
									.get(JsonVariable.MOBILEPHONE));
							user.setUpdatedBy((String) userObj
									.get(JsonVariable.UPDATED_BY));
							user.setUpdatedDate((Date) userObj
									.get(JsonVariable.UPDATED_DATE));
							user.setUsername((String) userObj
									.get(JsonVariable.USERNAME));
							user.setCifNumber((String) userObj
									.get(JsonVariable.CIF_NUMBER));
						...
						// User usr =
						// userService.findUserByUsername(request.getPostingFrom().getUsername());
						tf.setUsername(user);
						...
						ReceiverInfo receiver = receiverInfoService
								.findAccountByUsernameAndAccountNumber(user,
										request.getPostingTo()
												.getAccountNumber());
						...
						if (null != receiver) {
							...
							if (oldSave == true) {
								...
								receiver.setSave(request.getPostingTo()
										.isSave());
							}

							// exist
							log.debug("receiver "+request.getPostingTo().getAlias().trim());
							receiver.setAlias(request.getPostingTo().getAlias().trim());
							...
							if (request.getPostingTo().isSave() == true
									&& oldSave != true) {
								log.debug("send email receipt to "
										+ user.getEmail());
								transferService.sendEmailReceiptSaved(receiver,
										user, servletRequest.getLocale(),
										transferType);

							} else {
								log.debug("Not send email receipt to "
										+ user.getEmail() + " issave = false");
							...
							receiver.setAccountNumber(request.getPostingTo()
									.getAccountNumber());
							receiver.setAccountName(request.getPostingTo()
									.getAccountName());
							receiver.setAlias(request.getPostingTo().getAlias().trim());
							receiver.setCounter(1);
							receiver.setSave(request.getPostingTo().isSave());
							receiver.setUsername(user);
							Bank bank = bankService.findBankByBankCode(request
									.getPostingTo().getBankCode());
						...
						String noteReceiver = receiver.getAccountName().concat(
						...
						result.setResponseDescription((String) dataObj
								.get(JsonVariable.RESPONCE_DESCRIPTION));
						result.setStatusCode((String) dataObj
								.get(JsonVariable.RESPONSE_CODE));
						result.setStatusDesc((String) dataObj
								.get(JsonVariable.RESPONCE_DESCRIPTION));
					}
	...
	public InquiryAccountRes inquiryAmountViaAPI(InquiryAccountReq request) {
		...
		try {
			...
			if (res != null) {
				...
				JSONObject statusObj = (JSONObject) responseJson.get("Status"); // kenapa
																				// status
																				// ny
																				// beda
																				// gede
				...
				String statusCode = String.valueOf(statusObj.get("statusCode")); // handle
																					// statusCode
																					// in
																					// Long
				String statusDesc = (String) statusObj.get("statusDesc");
				...
				if ("200".equals(statusCode)) {
					...
					if ("00".equals(dataObj.get("responseCode"))) {
						result.setAmount(request.getAmount());
						result.setMessage(request.getMessage());
//						request.getPostingFrom().setAccountName(
//								request.getPostingFrom().getAccountName()
//										.toUpperCase());
						log.debug("sender name from inquiry "+(String) dataObj
								.get(JsonVariable.SENDER_NAME));
						request.getPostingFrom().setAccountName((String) dataObj
								.get(JsonVariable.SENDER_NAME));	
			
						result.setPostingFrom(request.getPostingFrom());

						PostingToReq postingTo = request.getPostingTo();
						postingTo.setAccountName((String) dataObj
								.get(JsonVariable.BENEFICIARY_NAME));
						result.setPostingTo(postingTo);
						result.setConfirmationCode((String) dataObj
								.get(JsonVariable.CONFIRMATION_CODE));
						result.setStan(stan);
						// result.setAdminFee(new
						// BigDecimal((String)dataObj.get(JsonVariable.AMOUNT_FEE)));
						// ambil dr db
						String adminFee = "";
						if (BkpmConstants.BUKOPIN_BANK_CODE.equals(request
								.getPostingTo().getBankCode())) {
						...
						String ref = request.getPostingFrom()
								.getAccountNumber()
								+ SEPARATOR_PIPE
								+ request.getPostingTo().getAccountNumber()
								+ SEPARATOR_PIPE
					...
					log.debug("error inquiry transfer " + statusCode + " - - "
							+ statusDesc);
				}
			}
		} catch (Exception e) {
	...
	public PostingElementRes postTransaction(PostingReq request) {
		...
		/*
		 * Elements elm = new Elements(); elm.setElement03("491010");
		 * elm.setElement04("255000"); elm.setElement06("55555555");
		 * elm.setElement07("1122334455"); elm.setElement10("");
		 * elm.setElement11("777777"); elm.setElement12("113507");
		 * elm.setElement13("1014"); elm.setElement18("6014");
		 * elm.setElement28(""); elm.setElement33("441");
		 * elm.setElement37("RR5SAV44L201"); elm.setElement41("TLR420202");
		 * elm.setElement48(
		 * "Andri Risya Kharisma - transfer debit kredit   - saving acc tutup - checking acc "
		 * ); elm.setElement49("360"); elm.setElement51("360");
		 * elm.setElement62(
		 * "000000000599999999991234567890123456789011223344556677889900TLR420202      000000FTdariT24           2223355555"
		 * ); elm.setElement100("441"); elm.setElement102("4217900020");
		 * elm.setElement103("4202900099");
		 */

		Elements elm = new Elements();
		elm.setElement03("49" + request.getPostingFrom().getAccountType()
				+ request.getPostingFrom().getAccountType());
		...
		elm.setElement07(transDateTime); // month,date,hours,minute,second
											// mmddhhMMss
		...
		/*
		 * LocalTime tm = request.getTimeLocal(); String time =
		 * String.valueOf(tm.getHour()) + String.valueOf(tm.getMinute()) +
		 * String.valueOf(tm.getSecond()); elm.setElement12(time); // time local
		 * HHMMSS
		 */
		/*
		 * LocalDate dt = request.getDateLocal(); String dateStr =
		 * String.valueOf
		 * (dt.getMonthValue())+String.valueOf(dt.getDayOfMonth());
		...
		elm.setElement33(BkpmConstants.FORWARDING_INSTITUTION_ID); // forwading
																	// institution
																	// code
		// elm.setElement37(request.getRetrievaReferencelNumber()); //retrieval
		// reference number
		elm.setElement41(request.getPostingFrom().getUsername()); // card
																	// acceptor
																	// (username)
		elm.setElement48(request.getMessage()); // additional data (transaction
												// message)
		elm.setElement49(BkpmConstants.CURRENCY_CODE_RUPIAH); // transaction
																// currency
		elm.setElement51(BkpmConstants.CURRENCY_CODE_RUPIAH); // currency code

		// generate application information
		/**
		 * 1-2 = fee code (dr??) 12= info saldo beda bank, 52=test in, 53=test
		 * out 3-8 = product code (dr??) SKN=109001, RTGS=109002 9-10 =
		 * operation code () 01= approval dg realisasi, 02= approval,
		 * 03=realisasi, 05=transaksi biasa, 06=transaksi reversal, 07=posting
		 * pending 11-20 = tanggal warkat (instrumen perbankan 000 10x) 21-40 =
		 * nomor warkat (nomor transaksi 000020x) 41-60 = device code 61-75 =
		 * user id (username / centage username) padding spasi di belakang(sisa
		 * diisi spasi) 76-78 = cabang (khusus teller 000 klo tidak ada) 79-81 =
		 * lokasi(khusus teller 000 klo tidak ada) 82-101 = FTdr T24 (diisi
		 * spasi) 102-104 = dep code (di isi spasi) 105-106 = flag via va (di
		 * isi spasi) 107-111 = flag via cbs (di isi spasi)
		 */
		elm.setElement62("000000000599999999991234567890123456789011223344556677889900TLR420202      000000FTdariT24           2223355555");
		elm.setElement100(BkpmConstants.FORWARDING_INSTITUTION_ID); // settlement
																	// id
		elm.setElement102(request.getPostingFrom().getAccountNumber()
				.toString()); // account identification (acc number from)
		elm.setElement103(request.getPostingTo().getAccountNumber().toString()); // account
																					// identification
																					// (acc
																					// number
																					// to)
		...
		try {
			...
			if (null != postingRS.value) {
				...
				if ("000".equalsIgnoreCase(postingRS.value.getISOMessages()
						.getDataElement().getElement39())) {
				...
				} else if ("094".equalsIgnoreCase(postingRS.value
						.getISOMessages().getDataElement().getElement39())) {
