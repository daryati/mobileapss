...
public class PLNPostpaidController {
	...
	private final Logger log = LoggerFactory
			.getLogger(PLNPostpaidController.class);
	 ...
	 * @throws IOException
	 *             Will returns Connection Exception.
	 */
	@PostMapping("/inquiry")
	public CommonResponse inquiryPostPaidPLN(
			@RequestBody CommonRequest<Map<String, String>> req)
			throws IOException {
		CommonResponse response = new CommonResponse();
		Calendar cal = Calendar.getInstance();
		String monthName = new DateFormatSymbols().getMonths()[cal
				.get(Calendar.MONTH)];
		...
		PrepaidInquiryRequest reqInquiryPostpaidPLNAranet = PLNUtils
				.generateInquiryPostpaidPLNReq(req.getData()
						.get("subscriberID"), forwardInsCode);
		log.debug("request : "
				+ BkpmUtil.convertToJson(reqInquiryPostpaidPLNAranet));
		PrepaidInquiryResponse resInquiryPostpaidPLNAranet = Services
				.create(PLNService.class)
				.prepaidInquiry(reqInquiryPostpaidPLNAranet).execute().body();

		log.debug("response : "
				+ BkpmUtil.convertToJson(resInquiryPostpaidPLNAranet
						.getRespayment().getResult()));

		String codeRes = resInquiryPostpaidPLNAranet.getRespayment()
		...
		if (PLN_CUT_OFF.equals(codeRes) || PLN_CUT_OFF_GIRO.equals(codeRes)) {
			...
			response.setMessage(messageUtil.get("error.user.pln.block",
					new Object[] { req.getData().get("subscriberID") },
					servletRequest.getLocale()));
			...
			String dataResp = resInquiryPostpaidPLNAranet.getRespayment()
			...
			if (billStatusInteger == 1) {
				...
				monthBillString1 = new DateFormatSymbols().getMonths()[Integer
						.parseInt(monthBill1) - 1];
				monthBillString1 = monthBillString1.substring(0, 3)
						.toUpperCase();
				...
				monthBillString2 = new DateFormatSymbols().getMonths()[Integer
						.parseInt(monthBill2) - 1];
				monthBillString2 = monthBillString2.substring(0, 3)
						.toUpperCase();
				...
				inqPostpaidResPLN.setAmount(new BigDecimal(totalElectricity1)
						.add(new BigDecimal(totalElectricity2)));
				inqPostpaidResPLN.setPenaltyFee(new BigDecimal(penaltyFee1)
						.add(new BigDecimal(penaltyFee2)));
				inqPostpaidResPLN.setPrevMeter(prevMeter1);
				...
				monthBillString3 = new DateFormatSymbols().getMonths()[Integer
						.parseInt(monthBill3) - 1];
				monthBillString3 = monthBillString3.substring(0, 3)
						.toUpperCase();
				...
				inqPostpaidResPLN.setAmount(new BigDecimal(totalElectricity1)
						.add(new BigDecimal(totalElectricity2)).add(
								new BigDecimal(totalElectricity3)));
				inqPostpaidResPLN.setPenaltyFee(new BigDecimal(penaltyFee1)
						.add(new BigDecimal(penaltyFee2)).add(
								new BigDecimal(penaltyFee3)));
				inqPostpaidResPLN.setPrevMeter(prevMeter1);
				...
				monthBillString4 = new DateFormatSymbols().getMonths()[Integer
						.parseInt(monthBill4) - 1];
				monthBillString4 = monthBillString4.substring(0, 3)
						.toUpperCase();
				...
				inqPostpaidResPLN.setAmount(new BigDecimal(totalElectricity1)
						...
						.add(new BigDecimal(totalElectricity4)));
				inqPostpaidResPLN.setPenaltyFee(new BigDecimal(penaltyFee1)
						.add(new BigDecimal(penaltyFee2))
						.add(new BigDecimal(penaltyFee3))
						.add(new BigDecimal(penaltyFee4)));
				inqPostpaidResPLN.setPrevMeter(prevMeter1);
				inqPostpaidResPLN.setCurrMeter(currMeter4);
			}

			inqPostpaidResPLN
					.setSubscriberID(req.getData().get("subscriberID"));
			...
			BigDecimal total = inqPostpaidResPLN.getAmount().add(
					inqPostpaidResPLN.getAdminFee().add(
							inqPostpaidResPLN.getPenaltyFee()));
			inqPostpaidResPLN.setTotalAmount(total);

			inqPostpaidResPLN.setElement11(resInquiryPostpaidPLNAranet
					.getRespayment().getResult().getElement11());

			StringBuilder sbElement48Req = new StringBuilder(dataResp);
			sbElement48Req = sbElement48Req.insert(19,
					dataResp.substring(19, 20));
	 ...
	 * 
	 * @param request
	 *            PLN Postpaid Request
	 * @return Http Response 200 with data in body, else Error.
	 * @throws IOException
	 *             Will returns Connection Exception.
	 */
	@PostMapping("/payment")
	public CommonResponse paymentPLN(
			@Valid @RequestBody CommonRequest<PLNPostpaidPaymentRequest> request)
			throws IOException {
		CommonResponse response = new CommonResponse(
		...
		log.debug("log dari limit harian pln postpaid " + lmtDl + " response "
				+ resLmtDL);
		if (resLmtDL.getCode().equals("000")) {
			// Call postpaid tibco
			String forwardInsCode = env
					.getProperty("config.pln.forwarding-institution-code");
			PostpaidRequest reqTibco = PLNUtils.generatePaymentPostpaidRequest(
					dataRequest, forwardInsCode, accType);
			log.debug("request to tibco : " + BkpmUtil.convertToJson(reqTibco));
			// get tibco response
			PostpaidResponse tibcoResponse = Services.create(PLNService.class)
					.postpaid(reqTibco).execute().body();
			log.debug("response from tibco : "
					+ BkpmUtil.convertToJson(tibcoResponse));
			String codeRes = tibcoResponse.getRespayment().getResult()
					.getElement39();
			// String postRes =
			// tibcoResponse.getRespayment().getResult().getElement121().substring(0,
			// 3);
			String monthName = new DateFormatSymbols().getMonths()[Calendar
			...
			if (PLN_CUT_OFF.equals(codeRes) || PLN_CUT_OFF_GIRO.equals(codeRes)) {
				...
				response.setMessage(messageUtil.get("error.id.pln.not.found",
					...
					|| SUBSCRIBER_SUSPENDED.equals(codeRes)) {
				log.error("Subscriber was blocked");
				response.setCode(ResponseMessage.PLN_SUBSCRIBER_SUSPENDED
						.getCode());
				response.setMessage(messageUtil.get("error.user.pln.block",
						new Object[] { dataRequest.getSubscriberNumber() },
						servletRequest.getLocale()));
			} else if (PLN_ALREADY_PAID.equals(codeRes)) {
				log.error("PLN already paid");
				...
				log.error("Current Bill not available");
				response.setCode(ResponseMessage.ERROR_BILL_NOT_AVAILABLE
						.getCode());
				response.setMessage(messageUtil.get("error.bill.not.available",
						new Object[] { monthName }, servletRequest.getLocale()));
				...
				response.setMessage(messageUtil.get(
						...
						servletRequest.getLocale()));
			} else if (PLN_INACTIVE_ACCOUNT_GIRO.equals(codeRes)
					|| PLN_INACTIVE_ACCOUNT_APPROVE_GIRO
							.equalsIgnoreCase(codeRes)
					|| PLN_INACTIVE_ACCOUNT_CLOSED_GIRO
							.equalsIgnoreCase(codeRes)) {
				...
				PLNPostpaidPaymentResponse data = PLNUtils
						.generatePaymentPostpaidResponse(tibcoResponse
								.getRespayment().getResult(), dataRequest);

				// Send receipt asynchronously
				plnService.sendEmailReceiptPLN(dataRequest, data, user);

				// Save data
				Transaction trx = saveTransaction(request.getIdentity(), data,
						username);
	 ...
	 */
	private Transaction saveTransaction(Identity identity,
			PLNPostpaidPaymentResponse data, String username)
			throws IOException {
		...
		if (ResponseMessage.SUCCESS.getCode().equals(res.getCode())) {
			...
			String bulan = data.getBillPeriod().values().stream()
