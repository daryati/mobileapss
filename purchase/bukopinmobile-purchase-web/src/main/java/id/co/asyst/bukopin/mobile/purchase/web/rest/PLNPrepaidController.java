...
public class PLNPrepaidController {
	...
	private final Logger log = LoggerFactory
			.getLogger(PLNPrepaidController.class);
	...
	// Giro
	private static final String PLN_EXCEED_LIMIT = "802"; // limit pembayaran
															// tidak cukup
	private static final String PLN_NOT_ENOUGH_BALANCE_GIRO = "805"; // salmin
																		// tidak
																		// cukup
	private static final String PLN_BLOCKED_ACCOUNT_DEBET = "806"; // Rekening
																	// diblok
																	// debet
	private static final String PLN_TRX_OVER_LIMIT = "808"; // Transaction over
															// limit
	...
	private static final String PLN_INACTIVE_ACCOUNT_GIRO = "822"; // Rekening
																	// inaktif
	private static final String PLN_USER_NOT_FOUND = "831"; // User Not found
	private static final String PLN_INACTIVE_ACCOUNT_APPROVE_GIRO = "839"; // Rekening
																			// belum
																			// disetujui
	private static final String PLN_TRX_DUPLICATE = "869"; // Transaction
															// duplicate
	private static final String PLN_INACTIVE_ACCOUNT_CLOSED_GIRO = "878"; // Rekening
																			// ditutup
	private static final String PLN_NOT_ENOUGH_BALANCE_VALUTA_GIRO = "885"; // Kode
																			// valuta
																			// tidak
																			// ada
																			// dalam
																			// batch
	private static final String PLN_NOT_ENOUGH_BALANCE_GIRO2 = "897"; // Saldo
																		// dibawah
																		// minimum
	 ...
	 */
	@PostMapping("/inquiry")
	public CommonResponse inquiryResult(
			@Valid @RequestBody CommonRequest<inquiryPrepaidRequestPLN> req)
			throws IOException {
		...
		String forwardInsCode = env
				.getProperty("spring.pln.forwarding-institution-code");

		PrepaidInquiryRequest reqInquiryPrepaidPLNAranet = PLNUtils
				.generateInquiryPrepaidPLNReq(req.getData().getSubscriberID(),
						forwardInsCode);
		log.debug("PLN prepaid req-inq to tibco: {}",
				BkpmUtil.convertToJson(reqInquiryPrepaidPLNAranet));
		PrepaidInquiryResponse resInquiryPrepaidPLNAranet = Services
				.create(PLNService.class)
				.prepaidInquiry(reqInquiryPrepaidPLNAranet).execute().body();
		log.debug("PLN prepaid resp-inq from tibco: {}",
				BkpmUtil.convertToJson(resInquiryPrepaidPLNAranet));

		String codeRes = resInquiryPrepaidPLNAranet.getRespayment().getResult()
				.getElement39();
		if (PLN_CUT_OFF.equals(codeRes) || PLN_CUT_OFF_GIRO.equals(codeRes)) {
		...
		} else if (UNREGISTERED_BILLER.equals(codeRes)
				|| UNREGISTERED_SUBSCRIBER_NUMBER.equals(codeRes)
			...
			response.setCode(ResponseMessage.DATA_NOT_FOUND.getCode());
			response.setMessage(messageUtil.get("error.no.meter.not.found",
					servletRequest.getLocale()));
		} else if (PRR_SUBSCRIBER.equals(codeRes)
				|| SUBSCRIBER_SUSPENDED.equals(codeRes)) {
			...
			response.setMessage(messageUtil.get("error.user.pln.block",
					new Object[] { req.getData().getSubscriberID() },
					servletRequest.getLocale()));
			...
			String dataResp = resInquiryPrepaidPLNAranet.getRespayment()
					.getResult().getElement48();
			...
			inqPrepaidResPLN.setTarifDaya(tarif.trim() + "/"
					+ new BigDecimal(daya));
			inqPrepaidResPLN.setNominal(req.getData().getNominal());
			inqPrepaidResPLN.setAdminFee(new BigDecimal(adminCharge));

			BigDecimal total = inqPrepaidResPLN.getNominal().add(
					inqPrepaidResPLN.getAdminFee());
			inqPrepaidResPLN.setTotal(total);

			inqPrepaidResPLN.setElement11(resInquiryPrepaidPLNAranet
					.getRespayment().getResult().getElement11());
			inqPrepaidResPLN.setElement48(dataResp);
			inqPrepaidResPLN.setElement62(resInquiryPrepaidPLNAranet
					.getRespayment().getResult().getElement62());
	...
	public CommonResponse inquirySubscriberName(@PathVariable String id)
			throws IOException {
		CommonResponse response = new CommonResponse();
		...
		String forwardInsCode = env
				.getProperty("spring.pln.forwarding-institution-code");
		PrepaidInquiryRequest reqInquiryPrepaidPLNAranet = PLNUtils
				.generateInquiryPrepaidPLNReq(id, forwardInsCode);
		log.debug("PLN prepaid req-inq to tibco: {}",
				BkpmUtil.convertToJson(reqInquiryPrepaidPLNAranet));
		PrepaidInquiryResponse resInquiryPrepaidPLNAranet = Services
				.create(PLNService.class)
		...
		if (PLN_CUT_OFF.equals(codeRes) || PLN_CUT_OFF_GIRO.equals(codeRes)) {
			...
			response.setMessage(messageUtil.get("error.user.pln.block",
					new Object[] { id }, servletRequest.getLocale()));
		} else if (PLN_EXCEED_LIMIT.equals(codeRes)
				|| PLN_TRX_OVER_LIMIT.equals(codeRes)) {
			log.error("exceed limit");
			response.setCode(ResponseMessage.LIMIT_TRANSFER_DAY.getCode());
	...
	public CommonResponse purchaseResult(
			@Valid @RequestBody CommonRequest<PurchasePrepaidRequestPLN> req)
		...
		response = Services
				...
				.verifyPIN(
						...
						commPinRequest).execute().body();
		if (!SUCCESS_CODE.equals(response.getCode())) {
			log.error("PIN Tidak Sesuai!");
			response.setCode(ResponseMessage.DATA_NOT_MATCH.getCode());
			response.setMessage(messageUtil.get("auth.pin.failed",
					servletRequest.getLocale()));
			return response;
		}

		// Validate Account Number's Owner
		VerifyAccountOwnerRequest verifyAccountData = new VerifyAccountOwnerRequest(
				username, accountNo);
		log.debug("Verify account owner: {}",
				BkpmUtil.convertToJson(verifyAccountData));
		...
		CommonResponse accountResponse = Services
				.create(UserModuleService.class)
				.verifyAccountOwner(verifyAccountRequest).execute().body();
		if (!ResponseMessage.SUCCESS.getCode()
				.equals(accountResponse.getCode())) {
			log.error("Error verify account owner: " + accountNo);
			// response not success
			return accountResponse;
		}
		ObjectMapper obMapper = new ObjectMapper();
		VerifyAccountOwnerResponse resAccount = obMapper.convertValue(
				accountResponse.getData(), VerifyAccountOwnerResponse.class);
		if (!resAccount.isValid()) {
			log.error("User and Account Info didn't match: "
					+ dataRequest.getAccountNo());
			String message = messageUtil.get("error.invalid.account.owner",
					servletRequest.getLocale());
		...
		lmtDLClass.setAccNo(accountNo);
		lmtDLClass.setUsername(username);
		lmtDLClass.setAmount(new BigDecimal(req.getData().getNominal() + ""));
		...
		log.debug("log dari limit harian pln prepaid " + lmtDl + " response "
				+ resLmtDL);
		if (resLmtDL.getCode().equals("000")) {
			PrepaidPurchaseRequest reqPurchasePrepaidPLNAranet = PLNUtils
					.generatePurchasePrepaidPLNReq(req.getData(),
							forwardInsCode, accType);
			log.debug("PLN prepaid req-purchase to tibco: {}", BkpmUtil
					.convertToJson(reqPurchasePrepaidPLNAranet.getParameter()));
			...
			resPurchasePrepaidPLNAranet = Services.create(PLNService.class)
					.prepaidPurchase(reqPurchasePrepaidPLNAranet).execute()
					.body();
			log.debug("PLN prepaid resp-purchase from tibco: {}", BkpmUtil
					.convertToJson(resPurchasePrepaidPLNAranet.getRespayment()
							.getResult()));
			String codeRes = resPurchasePrepaidPLNAranet.getRespayment()
					.getResult().getElement39();
			String postRes = resPurchasePrepaidPLNAranet.getRespayment()
					.getResult().getElement121().substring(0, 3);
			...
			if (PLN_CUT_OFF.equals(codeRes) || PLN_CUT_OFF_GIRO.equals(codeRes)) {
				...
				response.setMessage(messageUtil.get("error.cutoff.pln",
					...
					|| PLN_NOT_ENOUGH_BALANCE_GIRO.equals(codeRes)
				...
				response.setMessage(messageUtil.get("error.amount.not.enough",
					...
					|| PLN_INACTIVE_ACCOUNT_GIRO.equals(codeRes)
				...
				response.setMessage(messageUtil.get("error.duplicate.data",
					...
					&& !ARANET_TIME_OUT_CODE.equals(codeRes)) {
			...
			if (SUCCESS_CODE.equals(codeRes)) {
				...
				String dataResp = resPurchasePrepaidPLNAranet.getRespayment()
						.getResult().getElement48();
				String footNote = resPurchasePrepaidPLNAranet.getRespayment()
						.getResult().getElement63();
				String element122 = resPurchasePrepaidPLNAranet.getRespayment()
						.getResult().getElement122();
				purchasePrepaidResPLN = PLNUtils.generateResponsePurchase(
						dataResp, req.getData(), footNote, element122);
				response.setCode(ResponseMessage.SUCCESS.getCode());
				response.setMessage(messageUtil.get("success",
						servletRequest.getLocale()));
			...
			if (ARANET_TIME_OUT_CODE.equals(codeRes)
					&& SUCCESS_CODE.equals(postRes)) {
				response = callAdvice(req.getData());
				codeResAdvice = response.getCode();
				if (SUCCESS_CODE.equals(response.getCode())) {
					log.debug("resp Advice : " + response.getData());
					Map<String, String> dataResult = (Map<String, String>) response
							.getData();
					...
					purchasePrepaidResPLN = PLNUtils.generateResponsePurchase(
							resDataAdvice, req.getData(), footNote, null);
					response.setCode(ResponseMessage.SUCCESS.getCode());
					response.setMessage(messageUtil.get("success",
							servletRequest.getLocale()));
					response.setData(purchasePrepaidResPLN);
			...
			CommonResponse getUser = Services
					...
					.getUserByUsername(req.getData().getUsername()).execute()
					.body();
			if (null != getUser
					&& (SUCCESS_CODE.equals(codeRes) || SUCCESS_CODE
							.equals(codeResAdvice))) {
				...
				plnServices.sendEmailReceiptPLN(purchasePrepaidResPLN, resUser,
						servletRequest.getLocale());

				// save pln prepaid
				Transaction trx = savePurchase(req.getIdentity(),
						purchasePrepaidResPLN, resUser);
				purchasePrepaidResPLN.setDestinationId(String.valueOf(trx
	...
	/**
	 * Call this Advice Process if posting success but aranet is timeout. this
	 * advice process is to ask token number to tibco
	 * 
	 * @param req
	 * @return information after advice PLN
	 * @throws IOException
	 */
	public CommonResponse callAdvice(PurchasePrepaidRequestPLN req)
			throws IOException {
		...
		String forwardInsCode = env
				.getProperty("spring.pln.forwarding-institution-code");
		PrepaidAdviceRequest reqAdvicePrepaidPLNAranet = PLNUtils
				.generateAdvicePrepaidPLNReq(req, forwardInsCode);
		log.debug("request to aranet (Advice) : "
				+ BkpmUtil.convertToJson(reqAdvicePrepaidPLNAranet
						.getParameter()));
		PrepaidAdviceResponse resAdvicePrepaidPLNAranet = Services
				.create(PLNService.class)
				.prepaidAdvice(reqAdvicePrepaidPLNAranet).execute().body();
		log.debug("response (Advice) : "
				+ BkpmUtil.convertToJson(resAdvicePrepaidPLNAranet
						.getRespayment().getResult()));

		String codeResAdvice = resAdvicePrepaidPLNAranet.getRespayment()
				.getResult().getElement39();
		if (!SUCCESS_CODE.equals(codeResAdvice)) {
			log.error("Advice Error Code : "
					+ resAdvicePrepaidPLNAranet.getRespayment().getResult()
							.getElement39());
			response.setCode(ResponseMessage.ERROR_ADVICE_FAILED.getCode());
			response.setMessage(messageUtil.get("error.advice.failed",
					servletRequest.getLocale()));
		} else {
			log.info("Advice Success");
			String dataRespAdvice = resAdvicePrepaidPLNAranet.getRespayment()
					.getResult().getElement48();
			String footNote = resAdvicePrepaidPLNAranet.getRespayment()
					.getResult().getElement63();
	 ...
	 * 
	 * @param identity
	 *            The Identity object request
	 * @param data
	 *            The postpaid data to save
	 * @param username
	 *            The User's username
	 * @return The Transaction object data include destination ID will send as
	 *         response to frontend.
	 * @throws IOException
	 *             The Connection exception.
	 */
	public Transaction savePurchase(Identity identity,
			PurchasePrepaidResponsePLN resPln, Map<String, String> user)
			throws IOException {
		...
		log.debug("Call saveToFavouriteCommon: {}",
				BkpmUtil.convertToJson(destinationReq));
		CommonResponse res = Services.create(MasterModuleService.class)
		...
		if (ResponseMessage.SUCCESS.getCode().equals(res.getCode())) {
			transaction = oMapper
					.convertValue(res.getData(), Transaction.class);
