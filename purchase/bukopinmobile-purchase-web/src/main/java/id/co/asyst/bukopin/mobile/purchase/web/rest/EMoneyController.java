...
public class EMoneyController {
	...
	private static final BigDecimal MINIMUM_GOPAY_AMOUNT = new BigDecimal(
			"20000");
	private static final BigDecimal MAXIMUM_GOPAY_AMOUNT = new BigDecimal(
			"1000000");
	private static final BigDecimal MINIMUM_OVO_AMOUNT = new BigDecimal("10000");
	private static final BigDecimal MAXIMUM_OVO_AMOUNT = new BigDecimal(
			"2000000");
	...
	public CommonResponse inquiryGoPayResult(
			@Valid @RequestBody CommonRequest<inquiryEMoneyRequest> req)
			throws IOException {
		...
		EMoneyInquiryRequest reqInquiryEMoneyAranet = EMoneyUtils
				.generateInquiryEMoneyReq(req.getData().getCustomerNumber(),
						forwardInsCode, servletRequest.getLocale()
								.getLanguage(), req.getData().getType(), req
								.getData().getAmount());
		...
		if (isValid) {
			log.debug("request to TIBCO : "
					+ BkpmUtil.convertToJson(reqInquiryEMoneyAranet));
			EMoneyInquiryResponse resInquiryEMoneyAranet = Services
					.create(EMoneyModuleService.class)
					.emoneyInquiry(reqInquiryEMoneyAranet).execute().body();

			log.debug("response from TIBCO : "
					+ BkpmUtil.convertToJson(resInquiryEMoneyAranet));

			String codeRes = resInquiryEMoneyAranet.getRespayment().getResult()
					.getElement39();
			if (INVALID_AMOUNT.equals(codeRes)) {
				...
				response.setMessage(messageUtil.get(
						"error.id.emoney.not.found", servletRequest.getLocale()));
				...
				/*
				 * log.error("Error (result) PLN Postpaid: "+codeRes); // Throw
				 * middleware error throw new MiddlewareException(codeRes);
				 */
			} else {
				log.debug("Inquiry EMoney success");

				inquiryEMoneyResponse inqEMoneyRes = new inquiryEMoneyResponse();
				String name = resInquiryEMoneyAranet.getRespayment()
						.getResult().getElement61().substring(35, 65).trim();
				inqEMoneyRes.setCustomerName(name);
				inqEMoneyRes.setCustomerNumber(req.getData()
						.getCustomerNumber());
				inqEMoneyRes.setAmount(req.getData().getAmount());
				inqEMoneyRes.setElement37(resInquiryEMoneyAranet
						.getRespayment().getResult().getElement37());

				String amountFee = resInquiryEMoneyAranet.getRespayment()
						.getResult().getElement61().substring(78, 89);
				inqEMoneyRes.setAmountFee(new BigDecimal(amountFee));
				// add one character in element 61
				inqEMoneyRes
						.setElement61(resInquiryEMoneyAranet.getRespayment()
								.getResult().getElement61().concat("1"));
				Integer st = new Integer(resInquiryEMoneyAranet.getRespayment()
						.getResult().getElement11());
				inqEMoneyRes.setElement11(st.toString());
				inqEMoneyRes.setElement48(resInquiryEMoneyAranet
						.getRespayment().getResult().getElement48());
				inqEMoneyRes.setTotalAmount(new BigDecimal(amountFee).add(req
						.getData().getAmount()));
	...
	public CommonResponse purchaseGoPayResult(
			@Valid @RequestBody CommonRequest<PurchaseEMoneyRequest> req)
		...
		lmtDLClass.setAmount(req.getData().getAmount());
		...
		log.debug("log dari limit harian gopay " + lmtDl + " response "
				+ resLmtDL);
		if (resLmtDL.getCode().equals("000")) {
			EMoneyPurchaseRequest reqPurchaseEMoneyAranet = EMoneyUtils
					.generatePurchaseEMoneyReq(req.getData(), forwardInsCode,
							accType);
			log.debug("request to aranet : "
					+ BkpmUtil.convertToJson(reqPurchaseEMoneyAranet));

			EMoneyPurchaseResponse resPurchaseEMoneyAranet = new EMoneyPurchaseResponse();
			resPurchaseEMoneyAranet = Services
					.create(EMoneyModuleService.class)
					.emoneyPurchase(reqPurchaseEMoneyAranet).execute().body();
			log.debug("response from aranet: "
					+ BkpmUtil.convertToJson(resPurchaseEMoneyAranet));

			String codeRes = resPurchaseEMoneyAranet.getRespayment()
			...
			if (SUCCESS_CODE.equals(codeRes)) {
				...
				/*
				 * String dataResp =
				 * resPurchaseEMoneyAranet.getRespayment().getResult
				 * ().getElement48(); String footNote =
				 * resPurchaseEMoneyAranet.getRespayment
				 * ().getResult().getElement63();
				 */
				String element61 = resPurchaseEMoneyAranet.getRespayment()
						.getResult().getElement61();
				String element122 = resPurchaseEMoneyAranet.getRespayment()
						.getResult().getElement122();

				purchaseEMoneyRes.setAccountNumber(req.getData()
						.getAccountNumber());
				purchaseEMoneyRes.setAmount(req.getData().getAmount());

				BigDecimal adminFee = new BigDecimal(element61
						.substring(78, 89).trim());
				purchaseEMoneyRes.setAmountFee(adminFee);

				purchaseEMoneyRes.setCustomerName(element61.substring(35, 65)
						.trim());
				purchaseEMoneyRes.setCustomerNumber(req.getData()
						.getCustomerNumber());
				purchaseEMoneyRes.setDate(element61.substring(21, 29).trim());
				// purchaseEMoneyRes.setReference(element61.substring(89,
				// 109).trim());
				purchaseEMoneyRes.setReference(element122.substring(144, 159)
						.trim());
				purchaseEMoneyRes.setTime(element61.substring(29, 35).trim());
				purchaseEMoneyRes.setTotalAmount(req.getData().getAmount()
						.add(adminFee));
				...
				if (null != getUser && (SUCCESS_CODE.equals(codeRes))) {
					...
					eMoneyService.sendEmailReceiptEMoney(purchaseEMoneyRes,
							resUser, servletRequest.getLocale());
					/*
					...
					// save EMoney
					CommonResponse saveRes = savePurchaseEMoney(identity,
							purchaseEMoneyRes);
					...
					if (SUCCESS_CODE.equals(saveRes.getCode())) {
						...
						purchaseEMoneyRes.setDestinationId(saveRes.getData()
								.toString());
						response.setData(purchaseEMoneyRes);
					}
				} else {
	 ...
	 * 
	 * @param req
	 *            Inquiry Emoney Request
	 * @return
	 * @throws IOException
	 */
	@PostMapping("/inquiry/OVO")
	public CommonResponse inquiryOVO(
			@RequestBody CommonRequest<inquiryEMoneyRequest> req)
			throws IOException {
		CommonResponse response = new CommonResponse();
		...
		String forwardInsCode = env
				.getProperty("config.emoney.forwarding-institution-code");
		EMoneyInquiryRequest requestInquiry = EMoneyUtils
				.generateInquiryEMoneyReq(custNo, forwardInsCode,
						servletRequest.getLocale().getLanguage(), type, req
								.getData().getAmount());
		log.debug("Request inquiry OVO to tibco : "
				+ BkpmUtil.convertToJson(requestInquiry));

		EMoneyInquiryResponse responseInquiry = Services
				.create(EMoneyModuleService.class)
				.emoneyInquiry(requestInquiry).execute().body();
		log.debug("Response inquiry OVO from tibco : "
		...
		if (codeRes.equals(SUCCESS_CODE)) {
			...
			result = EMoneyUtils.generateInquiryOVOResponse(req.getData(),
					responseInquiry);
	 ...
	 * 
	 * @param req
	 *            Purchase EMoney Request
	 * @return
	 * @throws IOException
	 */
	@PostMapping("/purchase/OVO")
	public CommonResponse purchaseOVO(
			@RequestBody CommonRequest<PurchaseEMoneyRequest> req)
			throws IOException {
		CommonResponse response = new CommonResponse();
		...
		CommonResponse resLmtDL = Services.create(TransferModuleService.class)
				.verifydailylimit(lmtDl).execute().body();
		log.debug("log dari limit harian ovo " + lmtDl + " response "
				+ resLmtDL);
		if (resLmtDL.getCode().equals("000")) {
			// set request purchase OVO
			EMoneyPurchaseRequest purchaseOVOReq = EMoneyUtils
					.generatePurchaseEMoneyReq(req.getData(), forwardInsCode,
							accType);
			log.debug("Request purchase OVO to Tibco : "
					+ BkpmUtil.convertToJson(purchaseOVOReq));
			// call service purchase OVO from tibco
			EMoneyPurchaseResponse purchaseOVORes = Services
					.create(EMoneyModuleService.class)
					.emoneyPurchase(purchaseOVOReq).execute().body();
			log.debug("Response purchase OVO to Tibco : "
					+ BkpmUtil.convertToJson(purchaseOVORes));
			String codeRes = purchaseOVORes.getRespayment().getResult()
					.getElement39();

			if (SUCCESS_CODE.equals(codeRes)) {
				// set response purchase OVO
				PurchaseEMoneyResponse resp = new PurchaseEMoneyResponse();
				resp = EMoneyUtils.generatePurchaseOVOResponse(req.getData(),
						purchaseOVORes);
				...
				CommonResponse saveRes = savePurchaseEMoney(req.getIdentity(),
						resp);
				if (SUCCESS_CODE.equals(saveRes.getCode())) {
					log.debug("ID DESTINATION .... "
							+ saveRes.getData().toString());
